...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#note-count').textContent(), '1 post');
    ...
    for (const folder of ['active', 'recovery']) assert.equal(fs.readFileSync(path.join(home, folder, persisted[0].id, 'text.md'), 'utf8'), revised);
    ...
    await page.waitForFunction(() => document.querySelectorAll('.post').length === 0);
    assert.equal(await page.locator('#trash-count').textContent(), '1');
    await page.click('#open-trash');
    await page.screenshot({ path: path.join(home, 'trash.png'), animations: 'disabled', scale: 'css' });
    await page.getByRole('button', { name: 'Restore', exact: true }).click();
    await page.waitForFunction(() => document.querySelectorAll('.post').length === 1);
    await page.click('#close-trash');
    assert.equal(await page.locator('#trash-count').textContent(), '0');
