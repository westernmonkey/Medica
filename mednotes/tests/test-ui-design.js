...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.sidebar, .stats, .welcome, [data-view]').count(), 0);
    assert.equal(await page.locator('.panel').count(), 2);
    const panels = await page.locator('.panel').evaluateAll(elements => elements.map(el => { const r = el.getBoundingClientRect(); return { x: r.x, width: r.width, top: r.top, bottom: r.bottom }; }));
    assert.equal(panels[0].x, panels[1].x);
    assert.equal(panels[0].width, panels[1].width);
    assert.ok(panels[1].top > panels[0].bottom);
