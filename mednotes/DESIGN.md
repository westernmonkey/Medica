...
Use the real public/Medica-logo.png (copied unchanged into ui/assets for standalone Electron), Medica emerald, and a light paper-like canvas. The interface is one centered column: header, composer rectangle, and notes rectangle. No workspace sidebar, media category views, statistics, welcome banner, extra tools, or marketing slogans. Color and SVG ornament provide visual character without adding features.
...
Editing/tagging retains the working in-app dialog and draft-preserving failure states. Media capture, search, storage, and embedding behavior are unchanged.
