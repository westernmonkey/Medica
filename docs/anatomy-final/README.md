...
- At 100%, a virtualized, orthographic study board packs named structures by enabled system. Each square reserves 10 CSS px on each side, plus a small fitting margin. Tiles are normalized and labelled not to anatomical scale. Native scrolling/drag pans; buttons or Ctrl/Cmd-wheel resize tiles. At 0%, original coordinates are restored without modifying vertices. Inspection preserves board scroll position. Layout switches are immediate, including reduced motion.
