...
export function StudyBoard({manifest,enabled,hidden,onLayout,onSelect,onPrefetch}:{manifest:Manifest;enabled:string[];hidden:boolean;onLayout:(layout:StudyLayout,scroll:number,height:number,zoom:number)=>void;onSelect:(system:string,id:string)=>void;onPrefetch:(system:string)=>void}){
 ...
 useEffect(()=>{for(const system of new Set(visible.map(t=>manifest.structures.find(p=>p.id===t.id)!.primarySystem)))onPrefetch(system);},[layout,top,size.height,manifest,onPrefetch]); // eslint-disable-line react-hooks/exhaustive-deps
 return <div className="af-board" aria-hidden={hidden} style={{visibility:hidden?'hidden':'visible',pointerEvents:hidden?'none':'auto'}}>
